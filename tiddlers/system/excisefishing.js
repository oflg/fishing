...
exports['excisefishing'] = function (event, operation) {
  ...
  const fishtitle = event.paramObject.selectionAsAnswer === 'yes' ? (operation.selection.indexOf('^^?^^__') == -1 ? editTiddlerTitle + '/' + currenttime : operation.selection.split('^^?^^__')[0].replace('__', '')) : operation.selection.split('\n')[0];
  const fishtext = event.paramObject.selectionAsAnswer === 'yes' ? (operation.selection.indexOf('^^?^^__') == -1 ? operation.selection : operation.selection.replace('__' + fishtitle + '^^?^^__', '')) : operation.selection.replace(fishtitle, '').replace('\n', '');
